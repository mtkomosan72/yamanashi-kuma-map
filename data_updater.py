...
try:
    ...
    # 💡 罠の原因だった f""" をやめ、普通の文字列にすることでプロンプト内のカッコを完全無害化しました
    prompt = "以下の【山梨県庁ホームページのテキスト】から、最新の「熊の出没・目撃に関する状況」を最も新しい日付のものから1件だけ抜き出して、以下の【指定のJSON形式】で出力してください。\n" \
    ...
    if 'candidates' in res_json:
        ...
        # 💡 HTML全体をパニックの源である f-string から完全に隔離し、安全に結合する仕組みに変更しました
...
<head>
    ...
    <style>
        ...
        #filter-container select {
            padding: 5px; font-size: 14px; border-radius: 4px; border: 1px solid #ccc; cursor: pointer; font-weight: bold;
        }
    </style>
</head>
<body>
...
<script>
    ...
    loadMap("all");
        ...
        # 💡 3つの安全なパーツをガチャンと結合して一つのHTMLにします
        json_data_str = json.dumps(current_database, ensure_ascii=False, indent=4)
        new_html_content = html_top + json_data_str + html_bottom
